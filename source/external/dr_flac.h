...
static unsigned int drflac__read_bits(drflac* pFlac, unsigned int bitsToRead, unsigned char* pDataOut)
{
    ...
    // Part 1 - leftover bits
    if (pFlac->leftoverBitsRemaining > 0) {
        if (bitsToRead >= pFlac->leftoverBitsRemaining) {
            pDataOut[0] = (pFlac->leftoverByte & ~(0xFF << pFlac->leftoverBitsRemaining));
            ...
            pDataOut[0] = ((pFlac->leftoverByte & ~(0xFF << pFlac->leftoverBitsRemaining)) >> (pFlac->leftoverBitsRemaining - bitsToRead));
